...
- CORS permits https://buer.wonderelian.com and capacitor://localhost; public preflight returns 204 and disallowed origin returns 403. Frontend CSP permits only the named added API endpoint.
...
2026-09-22 iPhone fix: allow the packaged iOS origin capacitor://localhost in the dedicated API environment. Public preflight returned 204 with the exact origin; a real streamed chat completed with done. Other origins remain rejected.
