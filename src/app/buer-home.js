...
export function initBuerHome({getLanguage,setLanguage,openManual,getReport}) {
  ...
  function renderSuggestions(){
    ...
    for(const question of suggestions.slice(0,2)){const b=document.createElement('button');b.type='button';b.dataset.question=String(question.id);b.textContent=question[getLanguage()==='en'?'en':'zh'];const arrow=document.createElement('i');arrow.className='ph ph-arrow-right';arrow.setAttribute('aria-hidden','true');b.append(arrow);b.addEventListener('click',()=>{input.value=b.textContent;input.dispatchEvent(new Event('input'));input.focus({preventScroll:true});});container.append(b);}
