...
import {readGrowth,growthContext} from '../services/buer-growth.js';
...
export function initBuerHome({getLanguage,setLanguage,openManual,getReport}) {
  ...
  function refresh(){
    ...
    const action=readGrowth(localStorage).actions.find(x=>!x.done);const actionEl=$('#growthHomeAction');actionEl.replaceChildren();if(action){const small=document.createElement('small');small.textContent=getLanguage()==='zh'?'我的下一步':'MY NEXT STEP';const text=document.createElement('p');text.textContent=action.title;const open=document.createElement('button');open.type='button';open.className='growth-button';open.textContent=getLanguage()==='zh'?'记录进展':'Review progress';open.onclick=()=>void openManual();actionEl.append(small,text,open);}
  ...
  async function ask(question,retry=false){
    ...
    if(!retry){current.messages.push({role:'user',content:value,date:Date.now()});input.value='';input.style.height='';}
