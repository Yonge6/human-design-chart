...
- Browser acceptance: Google's real library emitted one page_view with analytics storage granted and advertising storage denied. Simulated form activity and page scrolling emitted no form, scroll, or chart events. A same-origin empty measurement frame keeps automatic detection away from product inputs; only the existing opt-in chart event is forwarded through a field allowlist. Validation collection requests were intercepted to avoid adding synthetic traffic to official reporting.
