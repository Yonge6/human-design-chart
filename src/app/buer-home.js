...
export function initBuerHome({getLanguage,setLanguage,openManual,getReport}) {
  ...
  async function ask(question,retry=false){
    ...
    try {
      ...
      const response=await fetch(endpoint(),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({messages:history,...(report?{report}:{}),...await chatAccess()}),signal:active.signal});
